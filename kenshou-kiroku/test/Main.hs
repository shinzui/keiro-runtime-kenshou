...
main = hspec do
  ...
  describe "kiroku bundle" do
    it "registers unique kiroku scenarios" do
      ...
      length [scenario | scenario <- scenarios, scenario.id.kind == Concurrency] `shouldBe` 10
      length [scenario | scenario <- scenarios, scenario.id.kind == Benchmark] `shouldBe` 3
