...
- `pgmq/ack/concurrency/random-sigkill-at-least-once` — random worker kills preserve at-least-once handling.
...
- `pgmq/effectful/concurrency/backend-termination-recovery` — backend termination is transient and the pool recovers.
...
- `pgmq/effectful/concurrency/network-partition` — reset, latency, blackhole bounds, and recovery through the fault proxy.
...
- `pgmq/notify/concurrency/listener-loss-poll-fallback` — polling bounds delivery during listener loss.
