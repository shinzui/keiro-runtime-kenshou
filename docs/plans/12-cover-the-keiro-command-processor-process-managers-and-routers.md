...
- [ ] Multi-process: `sigkill-crash-windows` passes all four park, kill, and restart variants with the production Kiroku adapter and an independent SQL oracle. A neighbouring transfer now completes once after each restart. Explicit `PMStateDuplicate` worker facts, `random-kill-exactly-once`, and `topologies` remain.
