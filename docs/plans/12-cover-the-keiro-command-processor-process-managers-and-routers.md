...
- [x] (2026-09-23T22:16:00Z) Create `kenshou-keiro/kenshou-keiro.cabal` with the library and the `kenshou-keiro-test` suite. The package, fixture modules, scenario modules, and suite build.
...
- [x] Write `.Fixture.Projection`, `.Fixture.Runtime`, `.Fixture.Bridge`. Inline and additive async projections, the subscription worker, store runner, list adapter, production Kiroku bridge, and configurable ack-stream bridge compile and run end to end. The runtime wrapper now carries optional tracer and Keiro metrics handles into command and worker options. The write-side telemetry scenario passed both in-memory/collect and off/off after switching to this wrapper.
...
- [x] (2026-09-23T21:37:00Z) Unit tests: codec round trip, generated model/transducer acceptance and balance agreement, workload determinism and worker identity separation, independent expected saga/router UUIDs with fixed witnesses, and list adapter acknowledgement log. Eleven examples pass, including two Hedgehog properties at 100 cases each.
- [x] (2026-09-23T22:16:00Z) Add `Kenshou.Suite.Keiro.bundle`, the scenario `keiro/command/correctness/fixture-roundtrip`, and the registration in `kenshou-cli`. The scenario passes with 100 accounts and 500 generated operations across deposits, withdrawals, transfer legs, and bonus declarations; SQL verifies the account and bonus logs, inline balances, and in-flight money accounting.
...
- [x] Command concurrency: `identical-commands-one-batch` passes with 16 simultaneous threads and with four real writer processes (`command.processes=4`); one process reports an append and three report a duplicate, with one durable event. `hot-stream-contention` passes with eight writers for 30 seconds. `sigkill-idempotent-resubmission` now paces deterministic deposit sequences across real processes, kills and restarts each selected writer from an overlap with its last acknowledged index, checks reported duplicates, and verifies every expected event ID occurs exactly once. Its full default run passed 3,600 operations across three processes and twelve kills with six durable checks. `model-based-parallel-commands` passes 100 generated concurrent histories over three accounts with linearizability and durable-log checks; failed runs retain Hedgehog shrink/replay evidence.
- [x] Snapshot correctness: `policy-matrix` passes for all five policies with durable snapshot row checks; `truncation-covering-snapshot` passes with covered, gapped, cleared, and uncovered streams. `seed-divergence-detection` passes at sample rates 1 and 0: the real writer reports a corrupted seed only when sampled and still appends the command. A separate instrumented command against the corrupted seed asserts that the divergence counter is one when sampled with metrics collected and zero when unsampled or metrics are off.
- [x] Projection scenarios: `async-dedup-and-fence` passes applied, duplicate, fenced, and post-prune outcomes with SQL table checks. `inline-atomicity-under-kill` passes the SIGKILL, backend termination, and projection SQL error arms with matching log and balance table before and after the fault. `async-at-least-once-under-kill` passes apply-before-checkpoint SIGKILL and restart with ten pending deposits at batch sizes 10 and 1. A three-kill batch-size-one run and a five-kill batch-size-ten run passed per-kill activity and bounded duplicate checks; the latter replayed one to five duplicates as the uncheckpointed prefix grew. Its seed-derived kill-position arm passed five crashes after 3, 5, 8, 10, and 11 cumulative applies with duplicate counts bounded by ten. Its `skip-dedup` arm fails three durable-effect verdicts. `async-apply-checkpoint-atomic` reproduces only its expected nonblocking failure against `mori://shinzui/keiro/okf/improvement-requests/concepts/IR-10` after the extension.
...
- [x] Correctness with the list adapter: `deterministic-ids-redelivery` passes for 50 transfers and three deliveries each; its unstable-name sabotage arm fails its verdict as intended. `timers-commit-with-manager-append` passes with an unchanged deadline after redelivery and a rejected second debit. `order-insensitive-join` passes with both input orders and strict halt/dead-letter policies. `policy-matrix` passes all nine combinations with callback, acknowledgement, durable effect, and dead-letter checks; `keiro.dispatch.poison` equals nine with metrics collected and zero with metrics off. `transient-classification` passes conflicting-credit, backend termination during a credit projection, malformed destination history, and mixed rejection/transient cells, plus a direct credit witness for `RetryExhausted 2 (WrongExpectedVersion …)`.
...
- [x] Real bridge: `retry-budget-dead-letter` passes on the production adapter at five deliveries and on `ack-stream` at a configured three deliveries. SQL confirms `max_attempts_exceeded` with the matching attempt count, the healthy neighbour completes, and two replay calls return `ReplayedFresh` then `ReplayedDuplicate` with one durable target credit. The Kiroku event bridge increments `keiro.subscription.deadlettered` once with metrics collected and zero with metrics off; a checkpoint query confirms advancement past both source transfers.
- [x] Multi-process: `sigkill-crash-windows` passes all four park, kill, and restart variants with the production Kiroku adapter and an independent SQL oracle. A neighbouring transfer completes once after each restart. The resumed worker replays an acknowledged signal and reports `PMStateDuplicate`; all four windows passed that fact. `topologies` passes two real workers as duplicate subscribers, consumer-group members, and lease-owned shard workers across ten transfers in both input orders, reporting the announce-first fraction and checking each saga's two events. The duplicate-subscriber and consumer-group arms require the same reported `PMStateDuplicate` fact; both passed seven checks, while the sharded arm passed six without that probe. `random-kill-exactly-once` passed a local 300-transfer, 30-second run with nine real worker restarts and four confirmed backend terminations. A default-rate run completed all 6,000 transfers with six durable checks passing, 60 restarts and 30 confirmed backend terminations, but missed the configured 50/s pace on this host. A rate-valid full-duration run at 20/s completed 2,400 transfers in 120.01 seconds with 29 restarts, 14 backend terminations, and all six durable checks passing. The scenario records effective submission rate and returns `inconclusive` when the open-loop driver overruns its schedule.
...
- [ ] Benchmarks: `throughput-latency` passes 15-second local two-writer runs for plain, SQL callback, and inline projection paths with sixteen accounts, configurable pool size, snapshot policy, memo length, and replay verification. A paced two-writer run with the default 1,000 accounts also passed. The inline path checks its balance table against the durable model. An unpaced run completed 10,811 commands with all four durable checks passing but exceeded the local driver CPU gate; paced runs passed. `hydration-cost` uses batched history preparation and Keiro commands at every snapshot boundary, then checks the prepared snapshot row and rejects close commands to keep the timed stream immutable. Both policy arms passed at length 100, and the `every-100` arm passed at lengths 1,000 and 10,000. The 10,000-event `never` arm passed four durable checks over 129 commands in 30 seconds, but had only 109 steady samples and an inconclusive measurement grade. `all-stream-append-ceiling` passes 15-second local runs at one and four independent writers with pool size four and durable ledger verdicts. Telemetry is wired to all three command benchmarks, and four throughput tracing/metrics combinations plus one hydrated in-memory/collect run passed before the new account and runner knobs. Process-manager `dispatch-latency` passed 15-second local list-adapter runs with telemetry off and in-memory/collect, measured samples, and durable saga and account checks. Its `pm.source=kiroku-adapter` arm passed a 15-second run with 1,399 transfers, 350 injected redeliveries, a benchmark grade, and all three SQL checks. Router `fanout-dispatch` passed a 40-second one-recipient list-adapter run; a ten-recipient two-worker run passed durable checks but exceeded the local driver CPU gate. Its `router.source=kiroku-adapter` arm passed a 40-second one-recipient run with 1,368 fanouts, 350 injected redeliveries, a benchmark grade, and all four SQL checks. Process knobs and the full hydration and ceiling matrices remain.
- [ ] Soaks: `seed-verification-backlog` is registered at full and reduced duration with stream length, sampling rate, telemetry, durable ledger checks, and a leak diagnosis. Batched history setup uses Keiro commands at snapshot boundaries and checks the final snapshot row. A one-minute rate-zero probe at stream length 10,000 completed 6,596 commands and passed four durable checks; its heap verdict lacked major collections. An opt-in `diagnose.major-gc-interval-ms` arm gathers post-major samples for a diagnostic soak, whose latency data is not comparable benchmark evidence. A three-minute rate-zero run completed 21,527 commands with four checks passing and showed 79.5 MB post-major growth. A matched two-minute constant-rate pair completed 391 commands in each arm, passed the durable checks, and showed nearly identical heap growth with seed verification off and on; both lacked enough steady samples for benchmark grading. The signal is recorded in `docs/findings/2-keiro-seed-backlog-heap-growth.md` pending longer isolation. Earlier one-minute stream-length-100 probes completed 4,889 commands at rate zero and 2,438 at rate one, with no command or ledger failures. `write-side-steady-state` is registered at full and reduced duration with two real writers, saga, router, projection, quiescence, and nine SQL checks. Its one-minute four-account probes passed all checks over 865 and 868 account events but had too few major collections and latency samples for a decision. Separate RTS and process series and leak reports cover all five children, and PostgreSQL connections are selected by each child's application name. A five-minute sixteen-account run passed all nine SQL checks over 9,250 account events and pruned old dedup rows, but returned failed because the process-manager and router child post-GC live heaps grew through the window. Native memory, threads, descriptors, and connections remained stable; the signal is recorded in `docs/findings/1-keiro-write-side-worker-heap-growth.md` pending isolation. An optional ten-second worker restart arm recorded six kills, passed ten recovery checks over 1,593 account events, and failed its main-process thread leak verdict. Retiring exited children did not remove that signal; see `docs/findings/3-keiro-steady-restart-harness-threads.md`. Full telemetry arms and longer runs remain.
...
- [x] (2026-09-24T00:50:00Z) A Keiro-only tier-`smoke` run plan selected sixteen scenarios and executed in about half a minute. Fifteen passed; `reaction-no-advance-receipt` reproduced its declared nonblocking `mori://shinzui/keiro/okf/adrs/concepts/ADR-41` failure. The plan executor exited zero with worst blocking outcome `Passed`.
...
- Decision: keiro's `ProcessManager.handle` is a pure function of the input event and cannot read manager state, so the "order-insensitive join" is modelled observationally: the saga's own state machine accepts `ObserveDebit` and `ObserveAnnounce` in either order, and a deliberately order-sensitive variant (`strictTransferManager`) exists only to show the worker halting when that rule is broken.
  Rationale: Verified in `mori://shinzui/keiro` (`keiro/src/Keiro/ProcessManager.hs`; source artifact URI pending): `handle :: input -> ProcessManagerAction ci targetCi`. Gating a dispatch on manager state is not expressible, so the expectation worth encoding is the one the module documentation states: every cross-stream join must be order-insensitive.
  Date: 2026-09-20
...
- Decision: Repeated command-writer crashes replay an overlap of acknowledged operations, while repeated projection crashes park after each successive durable apply.
  Rationale: The overlap covers an append whose acknowledgement was lost without assuming that process termination aligns with a command boundary. Parking the projection worker establishes each apply-before-checkpoint window before termination; checking the activity table after every kill proves that the worker advanced through distinct events rather than repeating one crash point.
  Date: 2026-09-24

- Decision: A post-GC heap-growth signal from the five-minute write-side soak is kept as a local investigating finding, without attaching an upstream known-defect reference yet.
  Rationale: The SQL effects passed, but process-manager and router live bytes rose while native memory and resource counts stayed stable. This distinguishes a measured signal from an isolated runtime bug and preserves the reproduction details in `docs/findings/1-keiro-write-side-worker-heap-growth.md` for the longer follow-up.
  Date: 2026-09-24

- Decision: The seed-backlog soak exposes an opt-in forced-major-GC arm and treats its latency samples as diagnostic only.
...
A Keiro-only smoke plan ran sixteen scenarios in about half a minute and exited
zero. Fifteen passed; the `NoAdvance` receipt scenario reproduced its declared
nonblocking known defect. The repeated writer-kill scenario passed its default
three-process run with 3,600 distinct operation IDs and twelve kills. Repeated
projection crashes passed at batch sizes one and ten; the skip-dedup sabotage
arm failed as intended. The five-minute reduced write-side soak passed all
nine durable checks but failed its leak verdict on process-manager and router
live-byte growth. The signal remains under investigation in
`docs/findings/1-keiro-write-side-worker-heap-growth.md`.

The 10,000-event seed-backlog setup and optional major-GC diagnostic arm passed
their durable checks. A matched pair at three offered commands per second
showed similar short-window heap growth with seed verification enabled and
disabled; `docs/findings/2-keiro-seed-backlog-heap-growth.md` records why the
retaining component remains unknown.

The integrated write-side soak now rotates real kills across the saga, router,
and projection workers while writers continue. A one-minute run passed ten
durable recovery checks over 1,593 account events and six restarts, but the
...
`keiro/command/concurrency/sigkill-idempotent-resubmission` (standard, either; durable only). Knobs: `command.processes` (int, 3), `fault.kill-interval-seconds` (int, 5), `command.duration-seconds` (int, 60), `client.resubmit-window` (int, 8), `command.rate-per-second` (int, 60). Procedure: writer roles run disjoint paced deposit sequences and report acknowledged indices; the scenario kills writers in round-robin order at each interval and restarts each from an overlap with its last acknowledged index, so operations that committed but were not acknowledged are resubmitted. Oracle: every expected operation ID occurs exactly once in the log, every final writer reports completion and duplicates, and the account balance agrees with the total accepted deposits (contract).
...
`keiro/projection/concurrency/async-at-least-once-under-kill` (standard, either; durable only). Knobs: `projection.batch-size` (int, 10, 1 to 100), `projection.events` (int, 10, 1 to 100), `projection.crash-count` (int, 1, 1 to 100, capped by the pending event count), `projection.sabotage` (enum, `none`; `skip-dedup`). Procedure: after seeding the account stream, park a projection worker after each of several successive applies, kill it, and restart another worker; an unarmed worker finally drains the backlog. Oracle: the activity table advances exactly once per apply at every crash boundary and equals the durable model at quiescence (contract); each restarted worker reports deduplication, with an `AsyncDuplicate` count between one and the batch size (implementation: Kiroku checkpoints per batch). With `skip-dedup` the scenario must fail.
...
`keiro/process-manager/benchmark/dispatch-latency` and `keiro/router/benchmark/fanout-dispatch`. Knobs: `pm.source` and `router.source` (`list`, `kiroku-adapter`), `pm.redelivery-percent` and `router.redelivery-percent` (int, 25, 0 to 100), `router.fanout` (int list, `1,10,100,1000`), and duration and load knobs. The production adapter arms inject one immediate retry for selected source events; their adapter observers measure handling from delivery to acknowledgement. Measurements: per-message handling time inside the worker, time from source append to worker completion, and the cost of a duplicate delivery relative to a fresh one.
