...
- [ ] Projection scenarios: `async-dedup-and-fence` passes applied, duplicate, fenced, and post-prune outcomes with SQL table checks. `inline-atomicity-under-kill` passes the SIGKILL, backend termination, and projection SQL error arms with matching log and balance table before and after the fault. `async-at-least-once-under-kill` passes apply-before-checkpoint SIGKILL and restart with ten pending deposits at batch sizes 10 and 1. A three-kill batch-size-one run and a five-kill batch-size-ten run passed per-kill activity and bounded duplicate checks; the latter replayed one to five duplicates as the uncheckpointed prefix grew. Its `skip-dedup` arm fails three durable-effect verdicts. `async-apply-checkpoint-atomic` still reproduces only its expected nonblocking failure against `mori://shinzui/keiro/okf/improvement-requests/concepts/IR-10` after the extension. Randomized kill timing remains.
