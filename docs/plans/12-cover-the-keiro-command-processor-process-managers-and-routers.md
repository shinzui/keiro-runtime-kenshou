...
- [ ] Projection scenarios: `async-dedup-and-fence` passes applied, duplicate, fenced, and post-prune outcomes with SQL table checks. `inline-atomicity-under-kill` passes the SIGKILL, backend termination, and projection SQL error arms with matching log and balance table before and after the fault. `async-at-least-once-under-kill` passes one deterministic apply-before-checkpoint SIGKILL and restart; its `skip-dedup` arm fails. `async-apply-checkpoint-atomic` reproduces only its expected nonblocking failure against `mori://shinzui/keiro/okf/improvement-requests/concepts/IR-10`. Repeated random kills and the batch-size bound remain.
