...
- [ ] Snapshot correctness: `policy-matrix` passes for all five policies with durable snapshot row checks; `truncation-covering-snapshot` passes with covered, gapped, cleared, and uncovered streams. `seed-divergence-detection` passes at sample rates 1 and 0: the real writer reports a corrupted seed only when sampled and still appends the command. Its metrics counter assertion remains pending telemetry wiring.
