...
- [ ] Soaks: `seed-verification-backlog` is registered at full and reduced duration with stream length, sampling rate, telemetry, durable ledger checks, and a leak diagnosis. One-minute local probes at stream length 100 completed 4,889 commands at rate zero and 2,438 at rate one, with no command or ledger failures. The rate-zero leak diagnosis lacked enough major collections; the rate-one run saturated the load driver and its short-window heap slope cannot establish a leak. `write-side-steady-state` is registered at full and reduced duration with two real writers, saga, router, projection, quiescence, and nine SQL checks. Its one-minute four-account probe passed all checks over 865 account events with projection dedup pruning invoked once per second. Separate RTS and process series and leak reports now cover all five children, and PostgreSQL connections are selected by each child's application name. A follow-up probe passed nine checks over 868 account events and produced first/last-tenth command latency p99s; 64 and 63 samples were below the decision minimum. Their one-minute leak verdicts lack enough major collections. The steady soak remains exploratory until optional kills, full telemetry arms, and longer runs are complete.
