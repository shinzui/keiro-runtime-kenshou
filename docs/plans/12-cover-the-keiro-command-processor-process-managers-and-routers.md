...
- [ ] Command concurrency: `identical-commands-one-batch` passes with 16 simultaneous clients; `hot-stream-contention` passes with eight writers for 30 seconds. `sigkill-idempotent-resubmission` passes with two real writer processes and a durable single-event oracle. `model-based-parallel-commands` passes 100 generated concurrent histories over three accounts with linearizability and durable-log checks; failed runs retain Hedgehog shrink/replay evidence. Multi-process support for `identical-commands-one-batch` remains.
