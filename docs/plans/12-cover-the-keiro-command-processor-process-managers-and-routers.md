...
- [ ] Multi-process: `sigkill-crash-windows` passes all four park, kill, and restart variants with the production Kiroku adapter and an independent SQL oracle. A neighbouring transfer completes once after each restart. `topologies` passes two real workers as duplicate subscribers, consumer-group members, and lease-owned shard workers across ten transfers in both input orders, reporting the announce-first fraction and checking each saga's two events. The duplicate-subscriber and consumer-group arms now request an acknowledged-signal replay in the worker and require its reported `PMStateDuplicate` fact; both passed seven checks, while the sharded arm passed six without that probe. `random-kill-exactly-once` passes a local 300-transfer, 30-second run with nine real worker restarts and four confirmed backend terminations. A full default run completed all 6,000 transfers with six durable checks passing, 60 restarts and 30 confirmed backend terminations; it took 249 seconds end to end, so its configured 50/s pacing is unproven on this host. The scenario now records effective submission rate and returns `inconclusive` when the open-loop driver overruns its schedule. Crash-window `PMStateDuplicate` facts and rate-valid full runs remain.
