...
- [ ] Command concurrency: `identical-commands-one-batch` passes with 16 simultaneous threads and with four real writer processes (`command.processes=4`); one process reports an append and three report a duplicate, with one durable event. `hot-stream-contention` passes with eight writers for 30 seconds. `sigkill-idempotent-resubmission` passes with two real writer processes and a durable single-event oracle. `model-based-parallel-commands` passes 100 generated concurrent histories over three accounts with linearizability and durable-log checks; failed runs retain Hedgehog shrink/replay evidence. The longer repeated-kill command workload remains.
