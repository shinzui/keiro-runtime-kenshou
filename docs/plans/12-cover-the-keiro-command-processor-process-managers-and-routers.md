...
`keiro/process-manager/benchmark/dispatch-latency` and `keiro/router/benchmark/fanout-dispatch`. Knobs: `pm.source` (`list`, `kiroku-adapter`), `pm.redelivery-percent` (int, 25, 0 to 100), `router.fanout` (int list, `1,10,100,1000`), `command.rate-per-second`. The production process-manager arm injects one immediate retry for selected source events; its adapter observer measures handling from delivery to acknowledgement. Measurements: per-message handling time inside the worker, time from source append to worker completion, and the cost of a duplicate delivery relative to a fresh one.
