...
- [ ] Benchmarks: `throughput-latency` passes a 15-second local two-writer run with measured samples, series, and three durable-log verdicts. `hydration-cost` passes local 15-second cases at stream length 100 with `never`/page 256 and `every-100`/page 64. It times a rejected close command so every sample hydrates the same immutable stream. The first two-second throughput run was inconclusive for insufficient samples; an unthrottled short run saturated the load driver. Runner, pool, telemetry, and process knobs remain for throughput, as do the full hydration matrix, `all-stream-append-ceiling`, process-manager `dispatch-latency`, and router `fanout-dispatch`.
