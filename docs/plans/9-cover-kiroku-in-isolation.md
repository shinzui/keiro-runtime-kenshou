...
- [ ] Append, read, lifecycle and transaction correctness scenarios (six complete: `expected-version-matrix`, `idempotent-event-ids`, `multi-stream-atomicity`, `all-order-and-gaps`, `cursor-semantics`, and `delete-and-truncate`; `append-with-continuation` still needs the enrich-hook comparison).
...
- [ ] All Milestone 1 scenarios pass with `pg.version=17` and `pg.version=18`; first draft of `docs/layers/kiroku.md` exists and covers the nine registered scenarios. The full eighteen-scenario validation remains. `cursor-semantics` passed on PostgreSQL 18 at page size 256 and PostgreSQL 17 at page size 7; `checkpoint-policies`, `filters-advance-checkpoint`, and `all-order-and-gaps` passed on both versions; `delete-and-truncate` and `append-with-continuation` passed on PostgreSQL 18. The gap report recorded zero missing positions before deletion and exactly the 400 deleted positions afterward.
