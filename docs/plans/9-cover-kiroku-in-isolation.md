...
- [ ] Subscription, consumer-group, dead-letter and notifier correctness scenarios (`checkpoint-policies` complete and passing on PostgreSQL 17 and 18; seven remain).
- [ ] Retention, metrics and otel correctness scenarios (three scenarios).
- [ ] All Milestone 1 scenarios pass with `pg.version=17` and `pg.version=18`; first draft of `docs/layers/kiroku.md` exists and covers the eight registered scenarios. The full eighteen-scenario validation remains. `cursor-semantics` passed on PostgreSQL 18 at page size 256 and PostgreSQL 17 at page size 7; `checkpoint-policies` passed on both versions; `delete-and-truncate`, `append-with-continuation`, and `all-order-and-gaps` passed on PostgreSQL 18. The gap report recorded zero missing positions before deletion and exactly the 400 deleted positions afterward.
