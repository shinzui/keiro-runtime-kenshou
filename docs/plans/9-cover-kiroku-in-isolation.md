...
- [ ] Subscription, consumer-group, dead-letter and notifier correctness scenarios (`checkpoint-policies`, `filters-advance-checkpoint`, and `retry-and-dead-letter` complete; five remain). The dead-letter default of three attempts passed on PostgreSQL 18 and the one-attempt variant passed on PostgreSQL 17.
- [ ] Retention, metrics and otel correctness scenarios (three scenarios).
- [ ] All Milestone 1 scenarios pass with `pg.version=17` and `pg.version=18`; first draft of `docs/layers/kiroku.md` exists and covers the ten registered scenarios. The full eighteen-scenario validation remains. `cursor-semantics` passed on PostgreSQL 18 at page size 256 and PostgreSQL 17 at page size 7; `checkpoint-policies`, `filters-advance-checkpoint`, `all-order-and-gaps`, and `append-with-continuation` passed on both versions; `retry-and-dead-letter` passed with three attempts on PostgreSQL 18 and one attempt on PostgreSQL 17; `delete-and-truncate` passed on PostgreSQL 18. The gap report recorded zero missing positions before deletion and exactly the 400 deleted positions afterward.
