...
- [ ] Subscription, consumer-group, dead-letter and notifier correctness scenarios (`checkpoint-policies`, `filters-advance-checkpoint`, `partition-coverage`, `retry-and-dead-letter`, `wake-latency`, and `category-idle-no-spin` complete; `catchup-live-handoff` passes its native API on both PostgreSQL versions but needs the Streamly API variants; `overflow-policies` remains). The idle category 90-second run on PostgreSQL 18 produced 44,290 events at 492 per second, with four live category fetches against a limit of five and 4,429 group fetches measured; a five-second PostgreSQL 17 run passed. The consumer-group scenario passed sizes 1, 2, 4, 8 for `$all` on PostgreSQL 18 and size 3 for `Category` on PostgreSQL 17. The dead-letter default of three attempts passed on PostgreSQL 18 and the one-attempt variant passed on PostgreSQL 17. The notifier scenario passed on both versions.
