...
- [ ] Subscription, consumer-group, dead-letter and notifier correctness scenarios (all eight registered; the native and both Streamly handoff paths now pass on both PostgreSQL versions). The default `overflow-policies` run passed on PostgreSQL 18 and 17 with 3,000 delivered events and a pause/resume pair; `DropSubscription` passed with a typed error and complete restart; `DropOldest` delivered 29 and reported 2,971 missing as an allowed measurement. The idle category 90-second run on PostgreSQL 18 produced 44,290 events at 492 per second, with four live category fetches against a limit of five and 4,429 group fetches measured; a five-second PostgreSQL 17 run passed. The consumer-group scenario passed sizes 1, 2, 4, 8 for `$all` on PostgreSQL 18 and size 3 for `Category` on PostgreSQL 17. The dead-letter default of three attempts passed on PostgreSQL 18 and the one-attempt variant passed on PostgreSQL 17. The notifier scenario passed on both versions.
- [ ] Retention, metrics and otel correctness scenarios (all three scenarios implemented; the full matrix remains). The metrics scenario passed `serve-scraped` on PostgreSQL 18 with three successful samples of each Kiroku endpoint and `serve` on PostgreSQL 17. The OTel scenario passed with tracing only on PostgreSQL 18 and with composed metrics collection on PostgreSQL 17.
- [ ] All Milestone 1 scenarios pass with `pg.version=17` and `pg.version=18`; first draft of `docs/layers/kiroku.md` exists and covers the eighteen registered scenarios. The full eighteen-scenario dual-version validation remains. `cursor-semantics` passed on PostgreSQL 18 at page size 256 and PostgreSQL 17 at page size 7; `checkpoint-policies`, `filters-advance-checkpoint`, `all-order-and-gaps`, `append-with-continuation`, `wake-latency`, default `overflow-policies`, native and Streamly `catchup-live-handoff`, and `endpoints-truthful` passed across both versions; `category-idle-no-spin` passed the full 90 seconds on PostgreSQL 18 and reduced five seconds on PostgreSQL 17; `partition-coverage` passed `$all` on PostgreSQL 18 and `Category` on PostgreSQL 17; `retry-and-dead-letter` passed with three attempts on PostgreSQL 18 and one attempt on PostgreSQL 17; `leases-block-hard-delete` passed with five seconds on PostgreSQL 18 and one second on PostgreSQL 17; `delete-and-truncate` passed on PostgreSQL 18. The gap report recorded zero missing positions before deletion and exactly the 400 deleted positions afterward.
