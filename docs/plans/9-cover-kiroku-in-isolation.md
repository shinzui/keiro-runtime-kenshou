...
- [ ] Append, read, lifecycle and transaction correctness scenarios (five complete: `expected-version-matrix`, `idempotent-event-ids`, `multi-stream-atomicity`, `cursor-semantics`, and `delete-and-truncate`; `all-order-and-gaps` checks 5,000 events and hard-delete gaps but still needs mixed multi-stream writes, native table counts and inventory comparison; `append-with-continuation` still needs the enrich-hook comparison).
- [ ] Subscription, consumer-group, dead-letter and notifier correctness scenarios (`checkpoint-policies` and `filters-advance-checkpoint` complete and passing on PostgreSQL 17 and 18; six remain).
- [ ] Retention, metrics and otel correctness scenarios (three scenarios).
- [ ] All Milestone 1 scenarios pass with `pg.version=17` and `pg.version=18`; first draft of `docs/layers/kiroku.md` exists and covers the nine registered scenarios. The full eighteen-scenario validation remains. `cursor-semantics` passed on PostgreSQL 18 at page size 256 and PostgreSQL 17 at page size 7; `checkpoint-policies` and `filters-advance-checkpoint` passed on both versions; `delete-and-truncate`, `append-with-continuation`, and `all-order-and-gaps` passed on PostgreSQL 18. The gap report recorded zero missing positions before deletion and exactly the 400 deleted positions afterward.
