...
- [ ] Known-defect scenarios (five) carrying `KnownDefect` references; `batch-size-validation`, `resize-leaves-gaps`, `decode-hook-stalls-subscribers` and `reconnect-cursor-regression` are registered and reproduced with `blocking=false` and process exit code 0. The decode-hook probe passed the full 60-second PostgreSQL 18 window and a reduced PostgreSQL 17 window; stall-watchdog diagnosis attachment remains. The reconnect probe passed its non-defect cells with 5,000 initial and 100 later events on both PostgreSQL versions; it recorded exactly 5,000 duplicates above the batch-size bound of 100. One known-defect scenario and plan-run validation remain.
...
- [ ] `Kenshou.Suite.Kiroku.Bench.Append` has `append-only` and `hot-stream` with pool, batch, writer and payload knobs and measurement-toolkit output; paced short local runs passed with raw samples, histograms and PostgreSQL series. A saturated short laptop run was correctly marked `infrastructure-failure` by the driver CPU health gate at 0.91 utilization. `expected-version-conflict` remains.
