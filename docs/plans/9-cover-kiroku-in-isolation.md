...
- [ ] Worker roles `kiroku.appender`, `kiroku.subscriber`, `kiroku.tx-appender` and `kiroku.reader` registered in the bundle and driven through the correctness toolkit's process control. The appender and reader roles are registered. The reader tails `$all` in a separate process during a concurrent write scenario; the subscriber and transaction appender roles remain.
