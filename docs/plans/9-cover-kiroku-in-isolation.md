...
- [ ] Known-defect scenarios (five) carrying `KnownDefect` references; `batch-size-validation`, `resize-leaves-gaps` and `decode-hook-stalls-subscribers` are registered and reproduced with `blocking=false` and process exit code 0. The decode-hook probe passed the full 60-second PostgreSQL 18 window and a reduced PostgreSQL 17 window; stall-watchdog diagnosis attachment remains. Two known-defect scenarios and plan-run validation remain.
