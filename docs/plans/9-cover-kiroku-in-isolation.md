...
- [ ] Retention, metrics and otel correctness scenarios (`leases-block-hard-delete` complete and passing on PostgreSQL 18 with a five-second lease and PostgreSQL 17 with a one-second lease; metrics and otel remain).
- [ ] All Milestone 1 scenarios pass with `pg.version=17` and `pg.version=18`; first draft of `docs/layers/kiroku.md` exists and covers the sixteen registered scenarios. The full eighteen-scenario validation remains. `cursor-semantics` passed on PostgreSQL 18 at page size 256 and PostgreSQL 17 at page size 7; `checkpoint-policies`, `filters-advance-checkpoint`, `all-order-and-gaps`, `append-with-continuation`, `wake-latency`, default `overflow-policies`, and native `catchup-live-handoff` passed on both versions; `category-idle-no-spin` passed the full 90 seconds on PostgreSQL 18 and reduced five seconds on PostgreSQL 17; `partition-coverage` passed `$all` on PostgreSQL 18 and `Category` on PostgreSQL 17; `retry-and-dead-letter` passed with three attempts on PostgreSQL 18 and one attempt on PostgreSQL 17; `leases-block-hard-delete` passed with five seconds on PostgreSQL 18 and one second on PostgreSQL 17; `delete-and-truncate` passed on PostgreSQL 18. The gap report recorded zero missing positions before deletion and exactly the 400 deleted positions afterward.
