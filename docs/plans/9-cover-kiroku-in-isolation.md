...
- [ ] Subscription, consumer-group, dead-letter and notifier correctness scenarios (`checkpoint-policies`, `filters-advance-checkpoint`, `partition-coverage`, `retry-and-dead-letter`, and `wake-latency` complete; three remain). The consumer-group scenario passed sizes 1, 2, 4, 8 for `$all` on PostgreSQL 18 and size 3 for `Category` on PostgreSQL 17. The dead-letter default of three attempts passed on PostgreSQL 18 and the one-attempt variant passed on PostgreSQL 17. The notifier scenario passed on both versions.
