...
- [x] (2026-09-24) Added named and ordinal sleeper definitions and `keiro/workflow/correctness/sleep-timer-smoke`. Both PostgreSQL durability modes passed seven verdicts for deterministic timer rows and payloads, stable first-arm deadline and wake hint, due discovery without firing, batched wake, completion journals, and single execution of surrounding step effects. Terminal-owner cancellation and generation rotation remain for the full `sleep-via-timers` scenario.
