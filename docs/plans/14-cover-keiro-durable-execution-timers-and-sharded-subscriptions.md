...
- [ ] Complete workflow role support for push modes, driver and GC roles, and telemetry handles. The resume role now uses Keiro's notifier for `push` and `neverWake` for `push-never-wake` and polling. The driver, GC and telemetry roles and the `push-lossy` source remain. The delivered kernel requires slash-form role names.
...
- [x] (2026-09-24) Added `keiro/workflow/concurrency/resume-workers-race`. Several real resume-worker processes race over deferred linear instances. Ten-instance runs passed in both PostgreSQL modes; a 100-instance durable run with four workers, pool size four and sixteen concurrent advances passed with one journal entry and one effect per step and no consumed retry attempts.
- [x] (2026-09-24) Added the first `keiro/workflow/concurrency/sigkill-step-boundary` arm. Two workers self-`SIGKILL` after flushed `s2` effects and one is killed by the supervisor after a flushed effect and before the step returns. A 20-instance shakedown and the default 100-instance durable run passed exact journal, terminal, zero-attempt, and crash-bounded effect checks; the default measured three duplicate effects. The longer periodic random-kill schedule and its fault knobs remain to be added.
- [x] (2026-09-24) Added the first `keiro/workflow/concurrency/crash-backoff-and-max-attempts` arm with `kenshouFlaky`. Two real workers drove four failures at the observed 2, 4 and 8 second spacing, remained active for sixteen quiet seconds after `WorkflowFailed`, then a repaired worker completed the resurrected instance. Durable PostgreSQL passed seven checks including retention of the failure marker. Backend-kill during an advance and the eight-attempt cap arm remain.
- [x] (2026-09-24) Added the backend-termination and postmaster-restart arms of `keiro/workflow/concurrency/database-faults`. It kills the active resume worker's PostgreSQL backends while a step effect is paused, restarts the run's PostgreSQL server, then starts two other processes. Both 20-instance and default 100-instance durable runs passed worker-survival, zero-attempt, exact-journal and at-least-once effect checks. Proxy reset, listener kill and repeated-fault scheduling remain.
- [x] (2026-09-24) Added `keiro/wake/correctness/push-fallback-when-notify-dropped`. It runs a simulated never-waking source, then a real push source whose `kiroku-listener` backend is terminated before signalling. Both modes passed at a one-second fallback; the durable run measured 1.01 s and 0.76 s signal-to-completion, each below `pollInterval + 2 s`.
...
## Revision Note — 2026-09-24 (shard database faults)

The shard worker now sends structured error-hook marks for reader, acquire,
snapshot and startup failures. The database-fault scenario terminates its
named PostgreSQL backends, restarts the run-owned postmaster, and requires an
acquire or reader error, a live worker, full recovered ownership, and all
