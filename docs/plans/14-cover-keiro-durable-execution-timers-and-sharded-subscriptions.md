...
- [x] (2026-09-24) Added the backend-termination arm of `keiro/workflow/concurrency/database-faults`. It kills the active resume worker's PostgreSQL backends while a step effect is paused, then starts two other processes. Both 20-instance and default 100-instance durable runs passed worker-survival, zero-attempt, exact-journal and at-least-once effect checks. Postmaster restart, proxy reset, listener kill and repeated-fault scheduling remain.
