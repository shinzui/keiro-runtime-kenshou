...
- [ ] `Kenshou.Measure.Samples`: `KSMP` version 1 raw-sample writer (per-worker blocks, single writer thread, back-pressure counter) and streaming reader.
- [ ] `Kenshou.Measure.Phase`: `PhasePlan`, `PhaseClock`, classification by intended start.
- [ ] `Kenshou.Measure.Recorder`: operations, per-worker recorders, per-cause error counters, merge and file output at finish, interval histograms when raw samples are not retained in full.
- [ ] `Kenshou.Measure.Session`, first part: `MeasureEnv` and `measureEnvFromRunContext`, the single point of contact with the kernel.
- [ ] Timing tests: warm-up samples are excluded from the steady histogram; a histogram recomputed from the raw file equals the recorded one; recording ten million samples on one thread stays under the cost budget.
...
`Kenshou.Measure.Cli` exports two `optparse-applicative` command parsers, and the only edit in `kenshou-cli` is to add them to the subcommand table the kernel plan created (find it with `grep -rn subparser kenshou-cli/`). `kenshou compare --baseline DIR --candidate DIR [more pairs] --policy FILE [--vary cohort|dim:NAME|knob:NAME] [--out FILE] [--json]` prints a table or the JSON document, writes it to `--out` when given, never writes into a run directory, and exits 0, 1, 3 or 4 by verdict and 2 for usage and compatibility errors. `kenshou summarize RUN_DIR [--json] [--verify]` prints the recomputed section; with `--verify` it compares every figure with the `measurements` section stored in `run-result.json` and exits 0 when they agree and 1 when they do not, which is the request's sixth acceptance criterion and the hook the evidence plan's verifier will call; an unreadable run directory exits 4.
