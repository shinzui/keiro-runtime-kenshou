...
- [ ] Implement `Kenshou.Suite.Shibuya.Fixture.SyntheticAdapter`, `.Handlers`, `.App`, `.RestartLoop` with unit tests. The synthetic broker now covers lease expiry, retry redelivery, stale finalization, scripted finalizer faults, throwing or blocking shutdowns, and a one-shot source fault followed by adapter replacement (17 package tests pass); handler, app, and restart helpers remain.
