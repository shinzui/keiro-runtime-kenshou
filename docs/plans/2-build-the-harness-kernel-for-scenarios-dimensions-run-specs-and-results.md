...
After this plan a maintainer can run `kenshou list` and see every registered scenario with its identifier, cost tier and placement, as text or as a versioned JSON document. `kenshou --help` groups the growing command tree by user intent, `kenshou help scenarios` explains the stable identifiers in prose that wraps to the current terminal but remains byte-stable when piped, `kenshou completions zsh` emits a completion script derived from the parser itself, and `kenshou --version` identifies the harness build. Typed operator defaults use Settei: `kenshou run --config local.yaml …` resolves built-ins below ordered YAML files below explicitly bound environment variables below named flags, and `--explain-config` reports the winning and shadowed origins without exposing secrets. They can run `kenshou run selftest/kernel/correctness/postgres-roundtrip --dim pg.durability=durable --seed 7 --out runs` and get a fresh run directory `runs/<run-id>/` holding exactly three contract documents — `run-spec.json` (what was asked, with every experiment-affecting default made explicit), `run-result.json` (the outcome, timings, the resolved identity of every runtime package, a fingerprint of the machine and the PostgreSQL server, and a compatibility key) and `manifest.json` (the SHA-256 digest of every file) — plus logs. The process exit code is 0 for passed, 1 for failed, 2 for a usage error, 3 for inconclusive and 4 for errored or infrastructure failure, so any script or the house CI platform can branch on it. Behind that command the kernel has started a private PostgreSQL server (version 17 or 18, with durability off or on as the dimensions ask), migrated the kiroku, keiro and PGMQ schemas into it through one migration ledger, handed the scenario a connection string, torn everything down, and hashed the results. A hidden `kenshou worker` subcommand lets a scenario run part of itself in a separate operating-system process of the same binary, which is what later crash tests need.
...
- Decision: Make the applicable `mori://shinzui/haskell-jitsurei` CLI patterns part of the `CliCommand` extension contract: intent-based command groups, `parserOptionGroup` for non-trivial commands, embedded help topics with terminal-aware width, parser-derived Bash/Zsh/Fish completions, explicit `-` document input, and clean stdout/stderr separation.
  Rationale: This plan is the single owner of a command tree extended by EP-3, EP-4, EP-6, EP-7, EP-17 and EP-18. Encoding interaction rules in the seam and tests prevents each plan from inventing a subtly different parser or machine-output convention. Hierarchical Dhall config is marked legacy by the catalog; aliases, FZF, clipboard integration and agent commands do not fit this deterministic harness and are excluded.
  Date: 2026-09-20

- Decision: Use the Settei 0.2.0.0 family for layered operator configuration, but keep all experiment-defining values in run specifications and other versioned documents.
  Rationale: `mori://shinzui/settei` provides typed declarations, deterministic precedence, provenance reports, strict YAML and secret-safe environment/CLI sources. A verification harness cannot permit ambient configuration to change a scenario invisibly, so scenario knobs, dimensions, seeds, policies and document inputs are outside the Settei layer; any configurable default that affects execution is copied into the effective run spec before the run begins.
  Date: 2026-09-20
...
The command line (Integration Point 6). This plan delivers `kenshou list`, `kenshou run`, `kenshou help`, `kenshou completions`, the hidden `kenshou worker`, and keeps `kenshou cohort show` plus EP-1's Git-aware `--version`. It owns the shared interaction standard described below: grouped commands and options, embedded terminal-aware topics, parser-derived completions, explicit standard-input document sources, machine-output channel discipline, and their tests. Exit codes: 0 passed, 1 failed, 2 usage error, 3 inconclusive, 4 errored or infrastructure-failure.
...
The CLI interaction standard comes from the active pattern catalog `mori://shinzui/haskell-jitsurei`. The exact applicable records are `mori://shinzui/haskell-jitsurei/docs/cli-option-groups`, `mori://shinzui/haskell-jitsurei/docs/cli-help-topics`, `mori://shinzui/haskell-jitsurei/docs/cli-help-width`, `mori://shinzui/haskell-jitsurei/docs/cli-shell-completions`, `mori://shinzui/haskell-jitsurei/docs/cli-stdin-integration`, and `mori://shinzui/haskell-jitsurei/docs/cli-version-git-sha`. The first five are implemented here; EP-1 supplies the version module. `mori://shinzui/haskell-jitsurei/docs/cli-overview` marks hierarchical Dhall configuration legacy and provides the basis for the explicit exclusions in the Decision Log.

Layered operator configuration comes from `mori://shinzui/settei`. Mori does not yet register its guide artifacts, so the precise references are that canonical project URI plus project-relative `docs/guides/cli-application.md` and `docs/guides/environment-and-cli.md`; their artifact-level URIs are pending. The implementation uses the released 0.2.0.0 packages `settei`, `settei-env`, `settei-optparse-applicative`, and `settei-yaml`. Hackage lists 0.2.0.0 as the latest release and upstream tag `v0.2.0.0` resolves as of 2026-09-20; recheck both under EP-1's fixed index before changing `^>=0.2.0.0`.

Two decisions of this plan deserve new ADRs. First, the one the MasterPlan assigns here: this repository, not `keiro-benchmarks`, owns the runtime-facing `list`/`run`/`compare` protocol, implementing IR-1 and serving UC-1, including the exit codes, the known-defect disposition and the CLI interaction standard. Second: scenarios never open a database themselves, the kernel provisions one private server per run, and all schema components share one ledger. Allocate each handle with `okf id next docs/adr --profile docs/adr/profile.dhall ADR`, name the file after the convention EP-1's records use, add a log entry with `okf log add`, and validate with `okf validate docs/adr --strict --profile docs/adr/profile.dhall --profile-enforce --log-enforce`.
...
`kenshou-core` gains a test suite `kenshou-core-test` (hspec with `hspec-hedgehog`). `kenshou-cli` becomes a library (`hs-source-dirs: src`), an executable `kenshou` (`app/Main.hs`, one line calling `Kenshou.Cli.Main.main`, `ghc-options: -threaded -rtsopts "-with-rtsopts=-N -T"`; `-T` enables the runtime statistics EP-4 samples) and `kenshou-cli-test`, which lists `build-tool-depends: kenshou-cli:kenshou` so the real binary is on `PATH` during tests. Its Cabal file lists `help/*.md` under `extra-source-files`, because Template Haskell embeds those files into the executable but Cabal must still ship them in a source distribution. Both cabal files follow the house shape of `mori://shinzui/keiro` at `keiro-test-support/keiro-test-support.cabal`: `cabal-version: 3.0`, a `common warnings` stanza, `default-language: GHC2024`, and `default-extensions: BlockArguments DuplicateRecordFields ImportQualifiedPost NoFieldSelectors OverloadedLabels OverloadedRecordDot OverloadedStrings`. With `NoFieldSelectors`, a field named `id` or `run` does not clash with the Prelude and is read as `scenario.id`. Sum types with more than one constructor use positional fields, because the house warning set includes `-Wpartial-fields`. Every type that appears in a versioned document gets hand-written `ToJSON` and `FromJSON` instances; enum spellings are given by explicit `render…`/`parse…` functions and never by `Show`.
...
Scope: the vocabulary of identifiers, the `Scenario` and `LayerBundle` types, a validated registry, the CLI skeleton and its extension seam, the shared help/completion/input conventions, and `kenshou list`. At the end `cabal run kenshou -- list` prints three self-test scenarios, `cabal run kenshou -- list --json` prints a `kenshou.scenario-list/v1` document, `kenshou help scenarios` renders an embedded guide, and `kenshou completions` emits scripts from the live parser; acceptance is those transcripts plus `cabal test kenshou-core:tests kenshou-cli:tests`.
...
data CliGroup = Discovery | Execution | Analysis | Evidence | Maintenance | Internal
...
parseInputSource :: String -> InputSource -- exactly "-" means InputStdin
...
`runCli` validates the registry (a `RegistryError` prints every problem and returns exit code 4), builds one `subparser` per visible group with `commandGroup`, one `internal` subparser for hidden commands, and parses with `execParserPure`. The visible groups and their order are Discovery, Execution, Analysis, Evidence and Maintenance. The hidden `worker` command uses Internal and appears in neither help nor generated completions. optparse-applicative's default exit code for a parse error is 1, which would collide with `failed`, so `runCli` handles `Failure` itself: render the message, and return `ExitSuccess` for `--help` or `ExitFailure 2` otherwise. A command signals a usage error by throwing `UsageError Text`, which `runCli` prints to standard error and turns into 2; any other escaped exception becomes 4. Every command parser with more than a few flags wraps intent-based record parsers in `parserOptionGroup` (for example Selection, Environment, Execution and Output), and every command and option supplies `progDesc` or `help` metadata so enriched completions have descriptions.

`InputSource` is the shared reader for document-valued `FILE` options. An explicit `-` reads standard input; omission never probes or consumes standard input, and a command with several document inputs rejects an invocation that assigns `-` to more than one of them. This adapts `mori://shinzui/haskell-jitsurei/docs/cli-stdin-integration` to a machine protocol where implicit pipe detection could make a non-interactive job hang. Every `--json` or other document-producing path writes only the requested bytes to standard output; progress, warnings and errors go to standard error. Tests run the real executable with captured handles and reject mixed output.

Create `kenshou-core/src/Kenshou/Core/Cli/Config.hs` as the dependency-neutral assembly point for persistent operator configuration. It defines a reusable `ConfigInputs` parser and resolver around Settei, with repeated `--config FILE` loaded by `Settei.Yaml.readYamlSource`, explicit `Settei.Env.Bindings`, named-option `Source` fragments from `Settei.Optparse.namedOption`, and the standard mutually exclusive diagnostic modes from `Settei.Optparse.diagnosticModeOptions`. The visible precedence, low to high, is built-in sources, YAML files in command-line occurrence order, one environment source built from a single startup snapshot, then named flags. Do not expose Settei's generic `--set`: kenshou already reserves that spelling for scenario knobs, and arbitrary structural overrides would make the reproducibility boundary hard to see. Reject unknown keys for the declaration being run; source/adapter failures exit 4 in kenshou's existing vocabulary, while optparse failures remain 2. `--describe-config` and `--describe-config-json` inspect a declaration without reading files or the environment; `--check-config`, `--explain-config`, and `--explain-config-json` load and resolve it. Requested JSON goes to stdout only on success; failures and a failed resolution's redacted explanation go to stderr with stdout empty. Keeping this generic seam in `kenshou-core` lets `kenshou-remote` and `kenshou-evidence` depend on it without importing `kenshou-cli`, which depends on them.

Create `kenshou-cli/src/Kenshou/Cli/Config.hs` for the first declaration, `runDefaultsConfig :: Settei.Config RunDefaults`, containing only operator conveniences already present in this plan: `run.output-root` (built-in `runs`), optional `postgres.pg17-bin`, and optional `postgres.pg18-bin`. Bind the binary paths explicitly to `KENSHOU_PG17_BIN` and `KENSHOU_PG18_BIN`; `--out` is the highest-precedence named source for the output root. `--cohort-identity` and `KENSHOU_COHORT_IDENTITY` remain an explicit versioned-document input and cell transport respectively, outside Settei. A connection string, if a later revision admits it to Settei, must be a `secretSetting`; the current `--pg-url-env` design is safer and remains. PostgreSQL binary paths and output roots are operational and go to the fingerprint or path metadata where applicable. Every resolved value that can affect execution is copied into the effective run specification before `executeRun`; no command consults Settei again during a run. Later plans may define their own small `Config a` declarations and reuse `Kenshou.Core.Cli.Config`, but they may not add scenario knobs, dimensions, seeds, policies, or versioned documents as ambient settings.

Create `kenshou-cli/src/Kenshou/Cli/Help.hs`, `Kenshou/Cli/Help/Width.hs` and `Kenshou/Cli/Completions.hs`. Help files `kenshou-cli/help/scenarios.md`, `selectors.md`, `run-specs.md`, `outcomes.md` and `exit-codes.md` are embedded with `Data.FileEmbed.embedStringFile`, registered as `HelpTopic` values and printed by `kenshou help [TOPIC] [--width COLUMNS]`; bare help prints a stable topic index. Lookup is case-insensitive. Explicit width wins. Otherwise `System.Console.Terminal.Size.size` is consulted only when standard output is a terminal, the detected width is capped at 140, and redirected output returns the embedded content byte-for-byte. The renderer preserves paragraphs whose non-empty lines begin with two spaces. Do not use `System.Console.ANSI.getTerminalSize`, which writes cursor-query escapes and may block. No FZF picker is added.

`kenshou completions bash|zsh|fish` emits static scripts that call the executable's built-in completion protocol: plain `--bash-completion-index` output for Bash and enriched `word<TAB>description` output for Zsh and Fish. The scripts contain no second command registry. Add golden tests for all three scripts and an integration test proving a newly registered fixture command and option appear through the completion protocol.

Create `kenshou-cli/src/Kenshou/Cli/Registry.hs` with `bundles :: [LayerBundle]` (initially `[Kenshou.Core.Selftest.bundle]`), `commands :: [CliCommand]` (list, run, worker, cohort, help, completions), and `topics :: [HelpTopic]`, and `kenshou-cli/src/Kenshou/Cli/Main.hs` with `main = getArgs >>= runCli bundles commands topics >>= exitWith`. Retain EP-1's `Kenshou.Cli.Version` and root `--version`. Move EP-1's cohort command into `kenshou-cli/src/Kenshou/Cli/Command/Cohort.hs` as a Maintenance `CliCommand` without changing its output. Put a comment block at the top of `Registry.hs` stating the rule: a coverage plan adds one import and one element to `bundles`; a toolkit or tool plan may add one import and one element to `commands`; a plan that adds a long-form guide adds one import and one element to `topics`. `list` and `help` are Discovery, `run` is Execution, `cohort` and `completions` are Maintenance, and `worker` is Internal.
...
Create `kenshou-cli/src/Kenshou/Cli/Command/Run.hs` with the full option surface, of which this milestone implements parsing, resolution and `--print-spec`: `kenshou run (--spec FILE | SCENARIO) [--config FILE]… [--describe-config|--describe-config-json|--check-config|--explain-config|--explain-config-json] [--set K=V]… [--dim N=V]… [--phase warm-up|steady|drain=SECONDS]… [--seed N] [--run-id UUID] [--timeout SECONDS] [--out DIR] [--pg-url URL | --pg-url-env VAR] [--placement local|cell] [--machine-profile NAME] [--cohort-identity FILE] [--cell-fingerprint FILE] [--keep-env] [--strict-known-defects] [--print-spec] [--json]`. Command-line values override the spec file, and named operational flags override Settei files/environment as described above. `--spec -`, `--cohort-identity -` and `--cell-fingerprint -` use `InputSource`, and an invocation may use `-` for only one of them; `--config -` is deliberately unsupported because it is not a versioned document and could contend for the same stream. The parser groups Scenario and run-spec selection under Input, knobs and dimensions under Parameters, placement and PostgreSQL under Environment, Settei paths under Configuration, diagnostic modes under Configuration diagnostics, seed/identifier/timeout under Execution, and output/printing under Output. `run.output-root` has the built-in value `runs`; `--out` is its highest-precedence source. Add `runs/` to `.gitignore`.
...
Milestone 1 is accepted when `kenshou list` prints the three scenarios sorted by identifier; `kenshou list 'selftest/*/correctness/*' --max-tier smoke --json` validates against the scenario-list schema once it exists; `kenshou list --bogus` exits 2 and `kenshou --help` exits 0 with the Discovery, Execution and Maintenance groups; and the unit tests show that `parseScenarioId` and `renderScenarioId` round-trip for generated identifiers, that identifiers with uppercase letters, three segments or an unknown layer or kind are rejected, that selectors match as described, and that `mkRegistry` reports each rule violation listed in Milestone 1 (one test per rule, each built from a deliberately broken bundle). CLI tests additionally prove that every visible command and option has help metadata, hidden `worker` appears in neither root help nor completion results, Bash/Zsh/Fish scripts are golden-stable and delegate to the parser protocol, redirected `kenshou help scenarios` is byte-identical to its embedded source, pseudo-terminals at 60, 100 and 200 columns wrap non-indented prose to 60, 100 and the 140-column cap, explicit `--width 50` wins, `--spec -` reads one JSON document, two `-` inputs exit 2 without consuming stdin, and `--json` never mixes diagnostics into standard output. Configuration tests additionally prove built-in < first YAML < later YAML < environment < named flag precedence; an invalid winning value does not fall back; duplicate/dotted YAML keys fail with locations; an unknown key fails; `--describe-config-json` performs no file or environment IO; failed explanations keep stdout empty; and a sentinel in a `secretSetting` appears nowhere in stdout, stderr, errors, warnings, or reports.
...
Libraries use EP-1's fixed Hackage index, with runtime bounds following the cohort's own cabal files: `base >=4.21 && <5`, `aeson >=2.2 && <2.3`, `bytestring`, `containers`, `text >=2.1 && <2.2`, `time`, `directory`, `filepath`, `unix >=2.8 && <2.9` (signals, user id and pseudo-terminal tests), `process` and `typed-process >=0.2.12 && <0.3` (workers), `async`, `stm`, `uuid >=1.3 && <1.4` and `mmzk-typeid >=0.7 && <0.8` (`Data.UUID.V7`), `splitmix` and `random >=1.2.1 && <1.4`, `cryptohash-sha256 >=0.11.102 && <0.12` and `base16-bytestring`, `optparse-applicative >=0.19 && <0.20`, `file-embed >=0.0.16 && <0.1`, `terminal-size >=0.3.4 && <0.4`, EP-1's `githash >=0.1.7 && <0.2`, `settei ^>=0.2.0.0`, `settei-env ^>=0.2.0.0`, `settei-optparse-applicative ^>=0.2.0.0`, `settei-yaml ^>=0.2.0.0`, `ephemeral-pg >=0.3.1 && <0.4`, `pg-migrate ^>=1.1.0.0`, `hasql >=1.10 && <1.11`, `hasql-pool`, `kiroku-store-migrations`, `keiro-migrations`, `pgmq-migration`, and, for the self-tests only, `kiroku-store >=0.8 && <0.9`, `keiro ^>=0.17.0.0`, `hasql-transaction`, `pgmq-core` and `pgmq-hasql` (0.6.1 family). Settei is a harness dependency and is deliberately absent from the runtime cohort identity. Tests use `hspec >=2.11 && <2.12`, `hedgehog >=1.4 && <1.8`, `hspec-hedgehog >=0.0 && <0.4` and `temporary`. Hackage on 2026-09-20 lists `file-embed` 0.0.16.0, `terminal-size` 0.3.4, and the four Settei packages at 0.2.0.0; the terminal-size 0.3.4 and Settei `v0.2.0.0` upstream tags exist, while file-embed's upstream tags stop at 0.0.15.0. Recheck the registry and tags during implementation, record any discrepancy, and prefer the Hackage release selected at EP-1's fixed index state rather than inventing a source pin. The dev shell gains `check-jsonschema`. Take exact runtime versions from `cohort/released.project`; do not loosen EP-1's constraints.

At the end of Milestone 1 these modules exist in `kenshou-core`: `Kenshou.Core.Id`, `.Outcome`, `.Selector`, `.Scenario`, `.Bundle`, `.Cli` (`CliCommand`, `CliGroup`, `HelpTopic`, `InputSource`), `.Cli.Config` (`ConfigInputs`, Settei source assembly and diagnostics), `.Version`, `.Selftest` (with type-only versions of `.Phase`, `.Knob`, `.Dimension`, `.Env`, `.RunSpec`, `.Context`, `.Role`), and in `kenshou-cli`: `Kenshou.Cli.Registry` (`bundles`, `commands`, `topics`), `Kenshou.Cli.Main`, `Kenshou.Cli.Config` (the run-default declaration and bindings), `Kenshou.Cli.Help`, `Kenshou.Cli.Help.Width`, `Kenshou.Cli.Completions`, EP-1's `Kenshou.Cli.Version`, `Kenshou.Cli.Command.List`, `Kenshou.Cli.Command.Cohort`, plus the five topic files under `kenshou-cli/help/`. At the end of Milestone 2: `Kenshou.Core.Phase`, `.Knob`, `.Dimension`, `.Env`, `.RunSpec`, `.RunSpec.Resolve` complete, and `Kenshou.Cli.Command.Run` with `runDefaultsConfig`, configuration diagnostics, and `--print-spec`. At the end of Milestone 3: `Kenshou.Core.Env.Migration`, `.Env.Postgres`, `.Role`, `.Role.Dispatch`, `.Role.Spawn`, and `Kenshou.Cli.Command.Worker`. At the end of Milestone 4: `Kenshou.Core.Log`, `.Context`, `.Fingerprint`, `.Canonical`, `.Compat`, `.RunResult`, `.Manifest`, `.Run`. At the end of Milestone 5: `schemas/`, the golden fixtures, and the seven self-test scenarios. The signatures are those given in Plan of Work.

What other plans consume. EP-3 uses `Registry`, `selectScenarios`, `ScenarioSelector`, `KnobSpec.variants`, `DimensionSupport`, `resolveRunSpec`, `EffectiveRunSpec` and its JSON form (a run plan is a list of them), `executeRun` or the `kenshou run --spec` command, `worstOutcome`, the completeness rule for run directories, and `CliCommand` for `plan` and `execute`. EP-4 uses `RunContext` (`withPhase`, `putSummary Measurements`, `artifactPath SamplesDir` and `SeriesDir`, `genFor`, `observe`), `ComparisonMembership`, `compatibleExcept`, `PostgresEnv.connectionString` for its samplers, and `CliCommand` for `compare` and `summarize`. EP-5 uses `WorkerRole`, the message types, `withWorker` as its spawn primitive, `ServerControl`, `PostgresEnv.adminConnectionString`, `putSummary Verdicts` and `artifactPath VerdictsDir`. EP-6 uses `putSummary Diagnosis`, `artifactPath DiagnosisDir` and `CliCommand`. EP-7 uses `Dimensions.tracing` and `.metrics`, `EnvironmentSpec.telemetry`, `putSummary Telemetry` and `compatibleExcept`. Every coverage plan (EP-8 to EP-15) uses `Scenario`, `LayerBundle`, `KnobSpec`, `DimensionSupport`, `EnvRequirements`, `RunContext`, `requirePostgres`, `newDatabase`, `KnownDefect`, and registers with one import, one `bundles` element and one `build-depends` line. EP-11 defines the content of `EnvironmentSpec.kafka` and reads `EnvRequirements.kafka`. EP-16 and EP-17 rely on the run-directory layout, `manifest.json`, `--cohort-identity`, `--cell-fingerprint`, `--pg-url-env`, `--placement cell` and the exit codes. EP-18 reads `run-result.json` and `manifest.json` and uses `verifyManifest`. Toolkits that add `selftest` scenarios export their own `LayerBundle` with `layer = Selftest`, using their toolkit name as the component segment (`measure`, `check`, `diagnose`, `telemetry`). Every later visible command also consumes `CliGroup`, `InputSource`, the option-group convention and stdout/stderr rules; EP-3, EP-4, EP-6, EP-17 and EP-18 add their `HelpTopic` values through `Kenshou.Cli.Registry.topics`. EP-17 and EP-18 additionally consume the dependency-neutral `Kenshou.Core.Cli.Config` resolver for their own narrowly scoped Settei declarations, so neither library depends on `kenshou-cli`.
