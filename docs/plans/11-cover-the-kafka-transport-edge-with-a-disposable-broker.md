...
- [ ] Milestone 2: the live AckOk, AckHalt, dead-letter, producer modes, transactional producer, and Keiro record-conversion checks pass; the two-topic partition-key and buffered-retry scenarios reproduce scoped, nonblocking counterexamples. The retry scenario's batch-size-one and early-exit controls pass. The batch-loop outage scenario reproduces its documented enqueue-versus-delivery limitation. Rebalance work remains. Milestone 3's buffered-successor ordering check reproduces its scoped counterexample.
