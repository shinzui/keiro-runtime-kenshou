...
- [ ] Deliver the disposable broker, Kafka adapter correctness and rebalance coverage, real crash/outage/model scenarios, benchmarks, soaks, and telemetry arms; verify the acceptance commands in Validation and Acceptance. The default 20,000-record, three-kill redelivery and static membership restart scenarios pass locally. The offset-store crash scenario reproduces KFK-5 with a passing manual-store control, and the released-cohort fencing scenario reports its scoped fatal-blindness counterexample.
