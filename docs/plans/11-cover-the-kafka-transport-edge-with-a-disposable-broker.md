...
- A two-lane partition run with a six-second session timeout confirmed B took over A's two partitions during A's twelve-second blackhole. All 2,000 acknowledged IDs had handler facts and sampled commits never regressed. Both workers then ended normally before stop, leaving lag 388 on each former A partition, consistent with BUG-4. It recorded 312 duplicates against an estimated bound of 241; because the worker lacks exact adapter buffer occupancy, the `zombie-duplicate-proxy-bound` failure remains blocking and is not filed as a separate bug. An earlier three-second-timeout probe was invalid with the worker's fixed two-second heartbeat and is excluded.
- The brokerless model runs the released adapter's real source and acknowledgement functions under a simulated `KafkaConsumer`. The default 2,000-case run violated all three declared properties with reproducible seeds and shrunk schedules. A fixed depth-ten script where offsets 3 and 4 retry once seeks first to 3 and then 4, never succeeds offset 3, and stops with stored offset 5 of log end 10. The independent depth-one reference handler passes the same properties. The model's released-cohort known-defect scope includes the finite-completion failure as well as commit safety and order; a separate live KFK-1 reproduction is still needed.
