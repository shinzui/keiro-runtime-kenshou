...
- [x] (2026-09-25) Implemented driver restart recovery in `mori://shinzui/load-testing-infra` at `04502e9`, the RFC 3339 recovery timestamp fix at `574c699`, and the recovery schema and contract at `45bdb26`. The Rust suite passed 33 library and three client tests, including recovery from an `accepted` status with a surviving result artifact; the checked Nix package build passed. A replacement driver image recovered the first live orphan `01a0d972-a5e1-71c2-a2ce-a8617c4d3677`: its status became sealed and `cellctl verify` passed four artifacts with manifest SHA-256 `c4e9cd4a50a0ba47f44741c5e138e4daaa346e21738a83286da988d2921dc1d1`. The recovery marker's timestamp was an array, however, and failed `cell.recovery/v1` schema validation; this immutable run is not schema-valid acceptance evidence. An attempted fresh run `01a0d9b1-2c63-76b1-9736-4c1d4128b57c` sealed `reset-failed` before execution because the older PostgreSQL role rejected the new optional `leaseSequence` status field. Its five-artifact tree verified and its journal recorded HTTP 500 and the exact decoding error. The matching PostgreSQL 18 role image and corrected driver image were deployed to `cell-alpha` at `fb65b03`, replacing one VM per upgrade while nine resources remained unchanged each time. A new run `01a0d9bc-d3d7-74ae-a291-0a116b2a4ac8` reached `running`; SIGKILL at 18:13:34 UTC terminated the driver, systemd restarted it at 18:13:40, and recovery sealed `infrastructure-failure` with reason `agent-restarted` at 18:13:41. The payload unit was `inactive`, result and recovery JSON passed their schemas, all four fetched artifacts passed `cellctl verify`, and terminal status matched manifest SHA-256 `1ac6a2e3d93c01c8625879f4ca3549e0c6cdedd4025ac7391a1301275cb1afcb`. The lease was released and all six cell VMs plus the x86 builder were confirmed `TERMINATED`.
- [x] (2026-09-25) Closed the upgrade admission window in `mori://shinzui/load-testing-infra`: `scripts/cell/upgrade.sh` now acquires and holds a dedicated lease through Pulumi apply, descriptor publication, and status readback, quarantines on failure, and releases on success. The first no-change probe stopped before GCP mutation because the Nix shell's Python lacks `uuid.uuid7()`; a portable version-seven generator replaced it. The live rerun on stopped `cell-alpha` reported ten unchanged resources; a concurrent lease contender returned exit 1 with `busy` and owner `cell-upgrade`; after the script exited, `status.sh` reported no lease and no quarantine. Its absent `state.json` now reads as unknown service health rather than a placeholder agent. Bash syntax and `git diff --check` passed.
- [x] (2026-09-25) Re-fetched SIGKILL recovery run `01a0d9bc-d3d7-74ae-a291-0a116b2a4ac8` after all cell VMs were stopped; `cellctl verify` passed the same four artifacts and manifest SHA-256 `1ac6a2e3d93c01c8625879f4ca3549e0c6cdedd4025ac7391a1301275cb1afcb`. A byte flipped in a separate local copy of `submission/work` made verification exit 1 and identify that file. Resubmitting the sealed run ID exited 4 with `run-id-already-used`; it did not require a lease and did not write a new work object. These close the stopped-cell durability, local tamper, and client-level run-ID reuse probes, while direct agent-credential overwrite refusal and later lifecycle transitions remain pending.
...
- Observation: an `OffsetDateTime` passed directly through `serde_json::json!` became a numeric tuple, although the recovery schema requires an RFC 3339 string. The first live recovered run's immutable marker cannot be repaired in place. A typed marker using `time::serde::rfc3339` fixed the serializer; the fresh SIGKILL run passed schema validation. No bug report is inferred from the local test alone.
  Evidence: `mori://shinzui/load-testing-infra` commit `574c699`; recovery marker from run `01a0d972-a5e1-71c2-a2ce-a8617c4d3677` failed `check-jsonschema`, while run `01a0d9bc-d3d7-74ae-a291-0a116b2a4ac8` passed on 2026-09-25.

- Observation: the new driver's optional `leaseSequence` status field caused the older PostgreSQL role's strict decoder to return HTTP 500 before reset. A matched role image resolved the live incompatibility. Future rolling upgrades need a coordination gate or a tolerant role status reader before clients can lease during replacement.
  Evidence: `mori://shinzui/load-testing-infra` run `01a0d9b1-2c63-76b1-9736-4c1d4128b57c` and its PostgreSQL agent journal, followed by the matched-image SIGKILL run above.
