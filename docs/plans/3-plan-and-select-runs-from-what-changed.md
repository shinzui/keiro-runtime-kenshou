...
- Decision: Contribute `plan` to the Discovery group, `execute` to the Execution group, and a `planning` help topic through EP-2's CLI registries; all document inputs use the shared explicit `-` convention and all complex parsers use intent-based option groups.
  Rationale: Planning is how users discover what should run, while execution performs the resulting work. Reusing the shared seam keeps help, completion, stdin and machine-output behavior consistent with every other kenshou command.
  Date: 2026-09-20
...
Create `kenshou-core/src/Kenshou/Plan/Summary.hs`, `kenshou-core/src/Kenshou/Plan/Execute.hs`, `kenshou-cli/src/Kenshou/Cli/Execute.hs` and `schemas/plan-summary.v1.schema.json`. Register `execute` as an Execution `CliCommand`. Its `--plan FILE` and `--environment FILE` options use EP-2's `InputSource`, with at most one `-`; group them under Input, put resume/fail-fast/timeout under Execution, and output-directory options under Output. Register `kenshou-cli/help/planning.md` as a `HelpTopic` that embeds the essential material from `docs/planning.md`: change sources, component closure, matrix policies, suites, plan identity and safe resume. The topic is the in-binary quick reference; `docs/planning.md` remains the full repository guide. The output directory of an execution has this shape, with EP-2's run directories as children:
...
Revision note (2026-09-20): Aligned `plan` and `execute` with EP-2's `haskell-jitsurei`-based CLI contract: user-intent command and option groups, explicit `-` document input, clean machine-output channels, and an embedded `planning` help topic.
