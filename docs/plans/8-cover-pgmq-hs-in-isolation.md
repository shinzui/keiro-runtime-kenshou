...
- [ ] Obtain stable reduced-soak leak verdicts with tracing off and OTLP. The tracing-off 100/s control now passed on the post-major-collection basis; OTLP and default-rate controls remain.
...
The layer is not yet complete. Seven of nine A/A controls need the quieter execution environment owned by EP-17, the wider outage acceptance still needs a sustained run, and the tracing-off 100/s reduced soak now has a stable post-major-collection heap verdict while OTLP and default-rate controls remain. Until those acceptance items are resolved, this plan and its MasterPlan registry row remain `In Progress`.
