...
- Observation: three other reproduced hazards lack a violated published provision claim. The blackhole experiment's ten-second limit is a harness bound; `pgmq-hs` does not promise an operation deadline. `QueueName` excludes mixed-case names, and the reconciler documents that foreign invalid metadata is left alone. PGMQ partition retention is configured by age or numeric distance, with no promise to inspect acknowledgement state before dropping a partition.
  Evidence: owner design note `mori://shinzui/pgmq-hs` at `docs/design/016-queue-name-validation.md` and shipped `mori://shinzui/pgmq-hs/okf/capabilities/concepts/CAP-1`; owner requests and plans `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-6`, `mori://shinzui/pgmq-hs/plans/24-report-name-collisions-and-unsupported-notifications-instead-of-acting-on-them`, and `mori://shinzui/pgmq-hs/plans/21-state-the-fifo-ordering-and-partitioned-retention-contracts-truthfully`.

- Observation: the network-blackhole scenario wrote its duration bound and its durability and recovery checks to one implementation-class verdict, although only the duration bound is an implementation expectation. It now writes a separate contract verdict for baseline, recovery, key conservation, and queue length, while the bound remains an implementation verdict under IR-6. The scenario report combines both failure lists, preserving exact known-defect matching so a new contract failure still blocks.
  Evidence: PostgreSQL 18 run `01a0d6f7-01c5-727d-a1ea-6ee69a7e44bf` sealed a held contract verdict and one implementation violation, `client-returned-within-bound`, in separate checker files. The scenario reference in `kenshou-pgmq/src/Kenshou/Suite/Pgmq/Concurrency/Outage.hs` names only that label.

- Observation: the previous 100/s tracing-off twenty-minute soak's apparent heap slope came from the periodic `live_bytes_last_gc` series, whose latest collection was usually generation zero. Sampling immediately after forced major collections gives a stable Haskell live heap at the same load while leaving the workload and queue controls intact. A stopped first attempt inherited 256 executors and was not comparable; the soak default is now eight executors and remains open-loop.
  Evidence: sealed PostgreSQL 18 run `01a0d6e3-6914-720c-b672-a4b19f480958` completed 120,000 cycles with zero failures, zero final queue rows and a stable leak verdict. Its heap probe used 39 post-major samples over 1,140 seconds, grew 26,104 bytes, and estimated 4,299 bytes/hour (95% interval -17,974 to 37,259). Aborted control `01a0d6e1-20bd-7593-9660-1a0e62c05320` had 256 executors and is excluded.

- Observation: the matched 100/s OTLP soak also sealed a stable leak verdict, so the corrected heap basis survives active trace export. PostgreSQL relation bytes grew during the finite run but are an informational probe; queue depth returned to zero and the archive stayed bounded.
  Evidence: sealed PostgreSQL 18 run `01a0d6f7-8feb-74d4-98fc-ec7b8326d90e` completed 120,000 cycles with zero operation failures and zero final queue rows. Its 39 post-major samples over 1,140 seconds grew 103,032 bytes, with a 260,398 bytes/hour slope (95% interval -88,892 to 637,322); the bounded heap verdict was `stable`. All 360,241 ended spans reached the sink, with zero drops or export failures; flush and shutdown succeeded.

- Observation: at the registered 500 cycles/s rate with tracing off, the corrected leak judge still passes without sacrificing the open-loop workload. The archive and relation-size probes are informational; the queue drained to zero.
  Evidence: sealed PostgreSQL 18 run `01a0d70a-9c05-762b-a69c-f54139287475` completed exactly 600,000 steady-state cycles with zero failures and zero final queue rows. Its 39 post-major samples over 1,140 seconds grew 28,936 bytes, with a 119,390 bytes/hour slope (95% interval 12,658 to 213,748); the bounded heap verdict was `stable`. The archive held 60,041 rows and the bloat verdict was `bounded`.

- Observation: the matched 500 cycles/s OTLP soak also passed. Tracing remained lossless at this rate and the queue drained after measurement. The informational relation-size probe grew during the run; the bounded live-heap probe stayed stable.
  Evidence: sealed PostgreSQL 18 run `01a0d71f-378e-7558-8f9d-ae4a64184647` recorded 600,001 successful steady cycles and zero operation failures, with zero final queue rows and a bounded archive of 60,043 rows. Its 39 post-major heap samples over 1,140 seconds grew 421,200 bytes, with a 752,044 bytes/hour slope (95% interval -25,903 to 1,169,509); the heap verdict was `stable`. All 1,801,445 ended spans were exported and received, with zero drops or export failures; flush and shutdown succeeded.

- Observation: repeating backend termination every ten seconds preserved confirmed deliveries and acknowledgements through twelve fault rounds on both PostgreSQL majors. The interrupted long poll still exposed the released classifier defect in each run. This exercise sends and drains a finite batch around each fault, so it does not yet establish the planned continuous-load oracle.
  Evidence: sealed PostgreSQL 18 run `01a0d73b-fb23-7612-a9cd-efb89084f29f` and PostgreSQL 17 run `01a0d73b-fb23-7101-b7dd-38940be8d681` each ran for 121.5 seconds, recorded twelve rounds and 2,400 delivered and acknowledged IDs, and found zero queue rows after every round. Each result had `blocking=false`, `knownDefect.status=reproduced` for `mori://shinzui/pgmq-hs/okf/bug-reports/concepts/BUG-1`, and only the `transient-error` failure label. The earlier 112-second versions `01a0d733-3e1f-721a-bc65-0f15648c157a` and `01a0d735-1e78-779e-9619-59ccf0278a52` prompted the final interval fix and are excluded from the duration claim.
...
- Observation: resetting a live proxied PostgreSQL connection can surface as `ServerError "" "" Nothing Nothing Nothing`, with no SQLSTATE. The same proxied pool recovers after forwarding resumes, but `Pgmq.Effectful.isTransient` classifies the empty server error as permanent.
  Evidence: run `01a0c6cd-bb2d-771a-9783-75da1b115a45` records the complete reset and recovery results in `summaries.verdicts.network-partition-observations`. Fresh PostgreSQL 18 reproduction `01a0d6d7-1f5e-768c-9b95-822031453720` and owner report `mori://shinzui/pgmq-hs/okf/bug-reports/concepts/BUG-1` subsequently closed the reporting gap; `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-4` tracks the repair.
...
- Observation: an in-process blackhole attempt exceeded the harness timeout because cancellation waited for the database call's non-interruptible I/O. Moving that call into a child process let the supervisor enforce a finite observation bound with `SIGKILL`. The call did not return within ten seconds even with libpq `tcp_user_timeout=5000`, while the same pool later recovered and both confirmed keys survived. The timeout option's behavior is consistent with a response-only blackhole, but the driver or kernel cause is not yet established.
  Evidence: initial stalled run `01a0cb40-6c31-7160-822b-03ac1930575e` required termination. Final PostgreSQL 18 runs `01a0cb43-f896-7260-bd2f-4522c619377a` (five-second default) and `01a0cb45-01c0-722a-b3e3-bb3c899c5d80` (ten-second bound, `tcp_user_timeout=5000`) and PostgreSQL 17 runs `01a0cb45-7444-7202-b320-d3f4d070cfa5` and `01a0cb45-d3e7-7182-9dc4-a03774192865` reproduced only `client-returned-within-bound` under `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-6`.
...
Three published-contract failures now have fresh reproductions, local findings, and validated owner bug reports. The classifier, concurrent-reconciliation report, and partitioned-notification scenarios cite those reports while their existing remediation requests and plans remain linked in the reports. The other reproduced hazards have contract-based non-bug dispositions.

The layer is not yet complete. Seven of nine A/A controls need the quieter execution environment owned by EP-17, the wider outage acceptance still needs a sustained run, and the two 100/s reduced soaks plus the 500/s tracing-off control have stable post-major-collection heap verdicts while the 500/s OTLP control remains. Until those acceptance items are resolved, this plan and its MasterPlan registry row remain `In Progress`.
...
`pgmq/effectful/concurrency/network-partition` (standard). The pool connects through the correctness toolkit's TCP proxy. Knobs `pgmq.fault.kind` (`reset` or `latency`; default `reset`) and `pgmq.fault.latency-ms` (default 200). Oracle: on reset, errors are transient and the pool recovers when the proxy heals, with confirmed payload keys conserved; on latency, no errors occur, latency rises by about two round trips, and all keys remain durable. Class `contract`. The reset classifier gap carries `mori://shinzui/pgmq-hs/okf/bug-reports/concepts/BUG-1`; its repair is tracked by `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-4`.

`pgmq/effectful/concurrency/network-blackhole` (standard). A producer worker process connects through the TCP proxy. Knobs `pgmq.conn.tcp-user-timeout-ms` (default 0, unset; passed as the libpq keyword `tcp_user_timeout` through `Hasql.Connection.Settings.other`) and `pgmq.fault.max-block-seconds` (default 30). After a confirmed baseline send, the proxy blackholes a request and response; the supervisor waits for the configured bound, records whether the call returned, and kills a stuck worker. It heals the proxy, sends again through the same pool, and checks durable payload keys. Oracle: the call returns within the bound, the pool recovers, and confirmed sends survive. The return bound is class `implementation` and carries `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-6`; all other checks remain blocking. A distinct scenario identifier keeps this timeout defect separate from the reset-classification defect.
