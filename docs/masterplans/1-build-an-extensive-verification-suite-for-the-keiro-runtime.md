...
- The existing EP-8 requests `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-4`, `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-5` and `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-6`, EP-9's blocking network-partition recovery finding, and EP-10's released-cohort findings need a report audit under Integration Point 11. Existing requests and review findings may supply context, but each confirmed broken provision claim needs a reproducible bug report unless an equivalent report already exists.
...
- EP-8 pgmq-hs findings: audit disconnect classification and concurrent reconciliation first, then the response blackhole, mixed-case alias collision, partitioned notification storm and retention loss against published behavior. Existing owner requests are `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-4`, `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-5` and `mori://shinzui/pgmq-hs/okf/improvement-requests/concepts/IR-6`; add local finding records and canonical bug concept URIs only for confirmed broken claims.
- EP-9 Kiroku findings: diagnose the blocking network-partition recovery miss separately from the known category replay, and audit the five reproduced known-defect scenarios against the existing Kiroku bug-report bundle and owner plans. Record each concept URI or non-bug disposition here; the network failure remains blocking meanwhile.
- EP-10 Shibuya findings: audit the reproduced historical 0.9.0.3 lifecycle failures and, as implemented, the current-release metrics and adapter failures against existing reviews, remediation plans and present bug reports. Record local findings and canonical concept URIs for confirmed broken claims; distinguish fixes already present in 0.10.0.0.
